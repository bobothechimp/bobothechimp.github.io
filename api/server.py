...
# Make all database tables if they don't exist
...
def createEvent(tournament_id: int, event_id: int):
    """
    Create and insert an event with the given event ID belonging to the given tournament ID.

    Parameters
    ----------
    tournament_id: int
        ID of the event's tournament
    event_id: int
        ID of the event
    ...
    -------
    int
        The status code of the creation and insertion of the event
    """
...
def addSets(event_id: int, sets: list, entrantsIdTable: dict):
    """
    Create and insert all sets from an event using an entrants ID table to facilitate operations.
    ...
    event_id: int
        ID of the event
    sets: list
        List of all sets from an event as generated by start.gg
    entrantsIdTable: dict
        Information about players, including their player ID, full name, gamer tag, team, seed, and placing

    Returns
    -------
    int
        The status code of the creation and insertion of the sets
    list
        The information for the biggest upset across all of the sets in the format of `[set id, upsetter seed, upsettee seed, UF]`
    """

    maxUF = [-1, 0, 0, 0]  # set id, upsetter seed, upsettee seed, UF
    status_code = CODES["SUCCESS"]
    for setData in sets:
...
def addPlayers(event_id: int, players: dict):
    """
    Create and insert all players from an event.
    ...
    event_id: int
        ID of the event
    players: dict
        List of all entrants from an event as generated by start.gg
    ...
    int
        The status code of the creation and insertion of the sets
    list
        The information for the biggest SPR across all of the sets in the format of `[player id, seed, placing, SPR]`
    """
...
def updatePlayersInEvent(event_id):
    """
    Update information for all players in an event.
    ...
    -------
    int
        The status code of the updating of the players
    """
