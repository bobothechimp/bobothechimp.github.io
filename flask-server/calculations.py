...
def determineBetterPlacing(curPlacing, oldTopPlacing):
    if curPlacing[1] < oldTopPlacing[1]:
        ...
        if curPlacing[1] == oldTopPlacing[1] and curEvent.entrants > oldEvent.entrants:
            return curPlacing
        else:
...
def highestUF(sets, entrantsIdTable):
    ...
    for set in sets:
        ...
        if uf > maxUF:
            maxUF = uf
            score = createScoreHeadline(
                set["displayScore"], winner_id, loser_id, entrantsIdTable
            )
...
def createScoreHeadline(displayScore, winner_id, loser_id, entrantsIdTable):
    ...
    return "{} {} - {} {}".format(
        winnerName,
        displayScore[winnerScoreIndex],
        displayScore[loserScoreIndex],
        loserName,
    )
...
def calculateDemonAndBlessing(player_id, winningSets, losingSets):
    ...
    for opponent_id in records.keys():
        record = records[
            opponent_id
        ]  # TODO stop players from having their demon be themselves
        """
        THE PROBLEM:
...
# Determines which player has played more sets
# Inputs should be lists whose indices correspond to the database table's columsn
def comparePlayerSets(player1, player2):
    return (player1[3] + player1[4]) - (player2[3] + player2[4])
